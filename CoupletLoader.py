...
try:
    ...
    if redis_client.ping():  # 测试连接 返回True表示连接成功
    ...
    if "unknown command" in str(e) or "FT._LIST" in str(e):
        ...
        print("不用担心！无需重新安装，我们编写的降级机制会检测到这一点，并在运行测试或主程序时自动切换为内存向量计算。")
    else:
        print("请检查你的 Redis 是否已启动。你可以通过以下命令在 Mac 上安装并启动 Redis：")
        print("  brew install redis")
        print("  brew services start redis")
    print("\n你可以直接运行 CoupletRetraval.py 或 Director.py，它们会自动在内存中进行相似度检索，不影响功能使用！")
